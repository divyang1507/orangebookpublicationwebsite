...
const CartPage = () => {
  ...
  const handleMockOrder = async () => {
    setMockLoading(true);
    try {
      const response = await fetch('/api/dev/mock-order', {
        method: 'POST',
      });
      const result = await response.json();

      if (!response.ok) {
        throw new Error(result.error || 'Failed to create mock order');
      }

      toast.success(`Mock order ${result.orderId} created! Cart cleared.`);
      await fetchCartItems(); // Refresh cart items (should be empty now)
      // Optionally redirect to an order confirmation page or user dashboard
      // router.push('/user/orders');

    } catch (err) {
      console.error("Mock order error:", err);
      toast.error(err.message || "Could not create mock order.");
    } finally {
        ...
        {cartItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 w-full md:w-2/5 lg:w-1/2">
                <img
                  src={item.images?.[0]?.url || '/placeholder-book.png'}
                  alt={item.name || 'Book cover'}
                  width="80"
                  height="100"
                  className="rounded object-cover flex-shrink-0 w-[80px] h-[100px] border"
                />
                <div className="flex-grow">
                  <Link href={`/product/${item.id}`}>
                    <h2 className="text-lg font-semibold text-gray-800 hover:text-orange-600 transition-colors duration-200 line-clamp-2">
      ...
      <div className="mt-10 border-t border-gray-200 pt-6 flex flex-col items-end space-y-3">
        ...
        {process.env.NODE_ENV === 'development' && (
          <Button
            ...
            className="mt-4 border-blue-500 text-blue-500 hover:bg-blue-50 hover:text-blue-600 px-8 py-3 rounded-lg text-base font-semibold"
          >
            {mockLoading ? 'Creating Mock Order...' : 'DEV: Create Mock Order'}
          </Button>
        )}
        {/* --- End Mock Order Button --- */}
